...
class Dem2Lines(Application):
  ...
  class Inventory(Application.Inventory):
    ...
    ## @li \b x_shift Value by which to shift x-values after processing.
    ## @li \b y_shift Value by which to shift y-values after processing.
    ...
    xShift.meta['tip'] = "Amount to x-shift values after processing."

    yShift = pyre.inventory.float("y_shift", default=0.0)
    yShift.meta['tip'] = "Amount to y-shift values after processing."
  ...
  def _configure(self):
    ...
    # Create ulines and vlines directories if not existing
    uLineDir = os.path.dirname(self.uLinePrefix)
    vLineDir = os.path.dirname(self.vLinePrefix)
    if not os.path.exists(uLineDir):
        print "ulines/ does not exist. Creating ..."
        os.makedirs(uLineDir)

    if not os.path.exists(vLineDir):
        print "vlines/ does not exist. Creating ..."
        os.makedirs(vLineDir)
  ...
  def _writeCubitJournals(self):
    ...
    numWidth = 4
    vfmt = "create vertex %15.11e %15.11e %15.11e\n"
    sfmt = "create curve spline vertex %d to %d delete\n"
    newLine = "\n"
    masterPref = "playback '"
    separator = "# ----------------------------------------------------------\n"
    comment = "#"
    ...
    self.xOut -= self.xShift
    self.yOut -= self.yShift
    ...
    m.write(comment3)
    exportCmd = "export Acis '" + self.acisFilename + "' overwrite\n"
    m.write(exportCmd)
    m.close()
    ...
    for row in range(self.numYOut):
      ...
      for column in range(self.numXOut):
        u.write(vfmt % (self.xOut[column], y, self.zOut[row, column]))

      vertBeg = vertNum
      vertEnd = vertBeg + self.numXOut - 1
      u.write(sfmt % (vertBeg, vertEnd))
      vertNum += self.numXOut
      u.close()
    ...
    for column in range(self.numXOut):
      ...
      for row in range(self.numYOut):
        v.write(vfmt % (x, self.yOut[row], self.zOut[row, column]))

      vertBeg = vertNum
      vertEnd = vertBeg + self.numYOut - 1
      v.write(sfmt % (vertBeg, vertEnd))
      vertNum += self.numYOut
      v.close()
  ...
  def _writeDemVtk(self):
    ...
    for yPoint in range(self.numYOut):
      for xPoint in range(self.numXOut):
        ...
        #v.write("%15.11e  " % self.zOut[xPoint, yPoint])
        if ((xPoint + 1)%5 == 0):
          v.write("\n")
